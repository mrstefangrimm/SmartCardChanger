...
function updateLiveVideoSettings() {
    let checkbox = document.getElementById("video-enabled");
...
function fetchLiveVideoAdjustments() {
    const applyButton = document.getElementById("apply_roi");
    fetch('/camera_adjustments')
        .then(response => response.json())
        .then(data => {
        let roi_x = document.getElementById("roi_x");
        let roi_y = document.getElementById("roi_y");
        let roi_width = document.getElementById("roi_width");
        let roi_height = document.getElementById("roi_height");
        // console.log('API Response:', data);  // Log the entire response
    ...
    applyButton.addEventListener('click', function () {
        const roi_x = document.getElementById("roi_x");
        const roi_y = document.getElementById("roi_y");
        const roi_width = document.getElementById("roi_width");
        const roi_height = document.getElementById("roi_height");
        if (!roi_x) {
...
function renderIntersections(intersections) {
    return intersections
        .map((pt, i) => `<li class="intersection-item">
